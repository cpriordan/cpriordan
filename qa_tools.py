...
class AdminLoginPage:
    ...
    def complete_2fa_login(self, test_env):
        ...
        if self.is_async:
            ...
            # Wait for redirect to admin/account home. Some account types (e.g. VYS admin)
            # land on "/" instead of "/admin/", so accept either instead of only "/admin/".
            import re
            try:
                self.page.wait_for_url(re.compile(f'https://{test_env}finalyticsdata.com(/|/admin/)'), timeout=45000)
                ...
                # If we're still stuck on the login page, the login itself didn't complete --
                # fail loudly here instead of letting every later step silently misbehave.
                if "/account/login" in self.page.url:
                    raise RuntimeError(
                        f"2FA login did not redirect away from the login page; still at {self.page.url}"
                    ) from e
    ...
    async def _complete_2fa_login_async(self, test_env):
        ...
        import re
        try:
            await self.page.wait_for_url(re.compile(f'https://{test_env}finalyticsdata.com(/|/admin/)'), timeout=45000)
            print(f"Successfully redirected to admin page: {self.page.url}")
        except Exception as e:
            print(f"Warning: Expected redirect to admin page failed: {e}")
            print(f"Current URL: {self.page.url}")
...
async def goto_with_retry(page, url, max_attempts=3, retry_sleep=5, **goto_kwargs):
    """
    Navigate to url, retrying on timeout/network errors up to max_attempts times.
    CI runners can see slower/flakier page loads than a local machine, so a bare
    page.goto() with no retry turns a transient slow load into a hard test failure.
    """
    last_error = None
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            last_error = e
            print(f"goto attempt {attempt}/{max_attempts} timed out navigating to {url}: {e}")
...
async def process_page_data_async(page, data_item, screenshots_directory, screenshot_counter, screenshots=True,
    ...
    if isinstance(data_item, int):
        ...
        await goto_with_retry(page, data_item)

...
class PostmanTestRunner:
    ...
    def validate_algorithm(self, expected_alg: str, actual_alg: str, product: str = '') -> bool:
        """Match algorithm validation like Postman utils.algCheck"""
        if not actual_alg:
            return False
        ...
        # Also check if product is mentioned (if specified)
        if product and product not in actual_alg:
            return False
    ...
    def process_response_like_postman(self, response_json: Dict, validation: Dict) -> Dict:
        ...
        for expected_alg in validation.get('expected_algorithms', []):
            ...
            found_alg = False
            for ad in ads:
                if self.validate_algorithm(expected_alg['algorithm'], ad.get('algorithms', ''), expected_alg['product']):
                    found_alg = True
                    break

...
def get_environment():
    """Prompt for environment selection and return (env_name, base_domain)."""
    # Accept as command-line arg or interactive prompt
    if len(sys.argv) > 1 and sys.argv[1].lower() in ("prod", "stg"):
        env = sys.argv[1].lower()
    else:
        while True:
            env = input("Enter environment [prod/stg]: ").strip().lower()
            if env in ("prod", "stg"):
                break
            print("  Please enter 'prod' or 'stg'.")

    if env == "prod":
        base_domain = "https://finalyticsdata.com"
    else:
        base_domain = "https://stgfinalyticsdata.com"
...
def check_ad_expiration():
    """Check ads on staging or production site for expiration status.

    1. Navigate to <base_domain>/qa/ad-expiration/
    ...
    env, base_domain = get_environment()
    base_url = f"{base_domain}/qa/ad-expiration/"
                    ...
                    if adcopy_link.count() > 0:
                        ...
                        if adcopy_href:
                            if not adcopy_href.startswith("http"):
                                adcopy_url = f"{base_domain}{adcopy_href}"
                            else:
    ...
    report_path = generate_html_report(ads_data, screenshots_dir, env, base_url)
    print(f"\nReport generated: {report_path}")
    print(f"Screenshots saved in: {screenshots_dir}")


def generate_html_report(ads_data, screenshots_dir, env, base_url):
...
<head>
    ...
    <title>Ad Expiration Report — {env_upper}</title>
    <style>
        body {{
...
<body>
    <div class="header">
        <h1>Ad Expiration Check Report <span class="env-badge">{env_upper}</span></h1>
        <p>Generated: {current_time}</p>
        <p>Source: <a href="{base_url}" style="color: white;">{base_url}</a></p>
    </div>
    ...
    report_path = os.path.join(
        ...
        f"test_ad_expiration_report_{env.upper()}.html"
    )

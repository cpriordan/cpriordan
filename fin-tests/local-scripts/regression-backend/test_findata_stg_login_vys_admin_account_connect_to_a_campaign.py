...
class UTMCampaignConnectorPage:
    ...
    def __init__(self, page):
        self.page = page
        # VYS admin account has a proper "Scenarios" left-hand nav menu
        self.scenarios_menu_link = page.get_by_text("Scenarios", exact=True).first
    ...
    def select_company(self, company_name, screenshots_directory):
        """Select the company context after login (VYS admin lands on company selection page)."""
        company_link = self.page.get_by_text(company_name, exact=True).first
        company_link.wait_for(state="visible", timeout=30000)
        company_link.click()
        print(f"Clicked '{company_name}' company link...")
        self.page.wait_for_load_state("networkidle")
        self.page.screenshot(path=f'{screenshots_directory}2_company_selected.png')

    def navigate_to_utm_connector(self, screenshots_directory):
        # VYS admin navigation: Scenarios → Create New Scenario → Connect to External/Paid Campaign
    ...
    def fill_campaign_form_and_submit(self, product_option, segment_option, screenshots_directory):
        ...
        self.page.screenshot(path=f'{screenshots_directory}3_form_waiting_for_products_menu.png')
        ...
        self.page.screenshot(path=f'{screenshots_directory}4_after_product_selection.png')

        # Wait for SEGMENTS menu to load
        self.page.wait_for_selector("#select2-segment-container")
        self.page.screenshot(path=f'{screenshots_directory}5_waiting_for_segments_menu.png')
        ...
        self.page.screenshot(path=f'{screenshots_directory}6_after_segment_selection.png')
...
def test_connect_to_a_campaign(admin_browser_context_sync):
    """Test connecting to a campaign using VYS admin account (has Scenarios menu permission)."""
    page = admin_browser_context_sync.new_page()

    # Use 'vys' admin account which has the Scenarios left-hand menu
    # (TRU staff account no longer has Scenarios menu permission)
    ...
    try:
        # Step: Select VYS company to set admin context
        utm_connector_page.select_company("VYS", screenshots_directory)

        # Navigate to Single Campaign URL form
        utm_connector_page.navigate_to_utm_connector(screenshots_directory)
        page.screenshot(path=f'{screenshots_directory}7_single_campaign_url_form.png')
        ...
        page.screenshot(path=f'{screenshots_directory}8_campaign_form_submitted.png')
        ...
        page.screenshot(path=f'{screenshots_directory}9_final_campaign_connection_state.png')

...
async def browser(request):
    ...
    async with async_playwright() as playwright:
        ...
        browser = await playwright.chromium.launch(headless=headless, args=["--remote-debugging-port=9222"])
...
async def validate_homepage_ad_with_retry(page, homepage_url, screenshots_directory,
                                          hero_heading_selector, hero_title_selector,
                                          expected_ad_heading, expected_ad_title):
    """
    Navigate to homepage_url and assert the expected heading and title are shown.
    Retries up to MAX_AD_RETRIES times (reload between each attempt) before raising.
    """
    last_error = None

    for attempt in range(1, MAX_AD_RETRIES + 1):
        print(f"[Homepage ad] Validation attempt {attempt}/{MAX_AD_RETRIES}")
        try:
            ...
            # The site's WP Consent plugin (#wpconsent-root / #wpconsent-container) sits on
            # top of the legacy cookie-consent banner and intercepts pointer events at its
            # location even though it reports zero height -- confirmed via
            # document.elementFromPoint() at the banner's click coordinates. Neutralize it
            # with pointer-events only (NOT display:none, which was found to cascade into
            # hiding the legacy banner too, presumably via shared consent-state JS) so clicks
            # pass through to the real cookie-dismiss button underneath.
            await page.evaluate("""
                () => {
                    for (const id of ['wpconsent-root', 'wpconsent-container']) {
            ...
            # Best-effort only: dismissing the cookie banner is not required for this test
            # (we only need to read the hero content), so this step is bounded to a few
            # seconds and can never block/consume the attempt's real time budget the way an
            # unbounded click-and-wait would if the banner is in an unexpected state (e.g.
            # visible-but-intercepted by the wpconsent overlay, or mid-transition to hidden).
            try:
                cookie_dismiss_button = page.locator('a.cc-btn.cc-dismiss', has_text="Close")
                if await cookie_dismiss_button.count() > 0:
                    await cookie_dismiss_button.click(force=True, timeout=5000)
                    print("Close cookie message button was found and clicked")
                else:
                    print("Close cookie message button not found")
            except Exception as cookie_exc:
                print(f"Cookie dismiss step skipped (non-fatal): {cookie_exc}")

            # Use Playwright's native selector wait instead of offsetHeight JS check,
            # which can incorrectly report height=0 in headless CI environments.
            await page.wait_for_selector(hero_heading_selector, state="attached", timeout=30000)
            await page.screenshot(path=f'{screenshots_directory}/hero_ad1_desktop_screenshot.png')

            ad_heading = await page.locator(hero_heading_selector).inner_text(timeout=10000)
            assert ad_heading == expected_ad_heading, (
                f"Ad heading '{ad_heading}' does not match expected '{expected_ad_heading}'"
            )
            print(f"Ad heading '{ad_heading}' matches expected heading '{expected_ad_heading}'")

            ad_title = await page.locator(hero_title_selector).inner_text(timeout=10000)
            assert ad_title == expected_ad_title, (
                f"Ad title '{ad_title}' does not match expected '{expected_ad_title}'"
            )
            print(f"Ad title '{ad_title}' matches expected title '{expected_ad_title}'")

            print(f"[Homepage ad] Correct ad found on attempt {attempt}.")
            return
        except BaseException as exc:
            if isinstance(exc, (KeyboardInterrupt, SystemExit)):
                raise
            last_error = exc
            print(f"[Homepage ad] Attempt {attempt} failed: {exc}")
            if attempt < MAX_AD_RETRIES:
                print(f"[Homepage ad] Waiting {RETRY_SLEEP}s before retry...")
                await asyncio.sleep(RETRY_SLEEP)

    raise Exception(
        f"[Homepage ad] Ad not found after {MAX_AD_RETRIES} attempts. Last error: {last_error}"
    )
...
async def test_missionfed_autoloan_stgtags_and_js_errors(
    browser,
    ...
    # Site DOM changed: .hero is no longer a direct child of #main (an extra wrapper div
    # was inserted), which broke the old rigid "> div > div > div" child-combinator chain
    # even though the personalized ad itself renders correctly and quickly. Switched to a
    # descendant selector anchored on stable class names instead of positional div nesting.
    hero_heading_selector="#main .hero .hero__content .pre-label",
    expected_ad_title="Your Next Car, No Guesswork.",
    hero_title_selector="#main .hero .hero__content h1.title",
    client="missionfed",
    ...
    try:
        ...
        await validate_homepage_ad_with_retry(
            page, homepage_url, screenshots_directory,
            hero_heading_selector, hero_title_selector,
            expected_ad_heading, expected_ad_title,
        )

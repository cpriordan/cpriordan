"""
Statewide OLB Card Ad CTA Links Test - PRODUCTION (GitHub Actions)

CI-compatible version of test_statewide_olb_card_ad_go_to_cta_links_PROD.py.

Changes from the local version:
- Runs headless (no browser window)
- Credentials sourced from environment variables / GitHub Secrets
- 2FA handled automatically via TOTP (pyotp) using STATEWIDE_OTP_SECRET
- Generates an HTML report saved as statewide_cta_links_report.html
- All output is CI-friendly (no Unicode that breaks Linux terminals)

Required GitHub Secrets:
    STATEWIDE_USERNAME      OLB login username
    STATEWIDE_PASSWORD      OLB login password
    STATEWIDE_OTP_SECRET    Base-32 TOTP secret (if account uses authenticator-app 2FA)
                            Leave unset if no 2FA is required in the test environment.

Optional environment variables (set in workflow or repo variables):
    STATEWIDE_BASE_URL      Default: https://statewide.bankjoy.com
    STATEWIDE_SKIP_2FA      Set to "true" to skip OTP handling entirely
"""
...
# =============================================================================
# CONFIGURATION — all sensitive values come from environment
# =============================================================================
BASE_URL      = os.environ.get("STATEWIDE_BASE_URL", "https://statewide.bankjoy.com")
USERNAME      = os.environ.get("STATEWIDE_USERNAME", "")
PASSWORD      = os.environ.get("STATEWIDE_PASSWORD", "")
OTP_SECRET    = os.environ.get("STATEWIDE_OTP_SECRET", "")
SKIP_2FA      = os.environ.get("STATEWIDE_SKIP_2FA", "false").lower() == "true"

LOGIN_URL     = f"{BASE_URL}/?cb=1&session_init=1&debug_all=1"
DASHBOARD_URL = f"{BASE_URL}/consumer/main/dashboard"

REPORT_PATH   = Path("statewide_cta_links_report.html")
SCREENSHOT_ROOT = Path("statewide/olb_PROD/cta_links")
...
async def handle_otp(page) -> bool:
    ...
    Uses pyotp with STATEWIDE_OTP_SECRET if available.
    Returns True if OTP was submitted (or not needed), False on failure.
    """
    if SKIP_2FA:
        print("[CI] STATEWIDE_SKIP_2FA=true — skipping OTP handling")
        return True

    # Check if an OTP input is visible
    otp_locator = page.locator(SELECTORS["otp_input"])
    try:
        await otp_locator.first.wait_for(state="visible", timeout=8000)
    except PlaywrightTimeoutError:
        print("[CI] No OTP input detected — assuming 2FA not required")
        return True
    ...
    if not OTP_SECRET:
        print("[CI] ERROR: STATEWIDE_OTP_SECRET is not set but OTP prompt appeared")
        return False
...
async def perform_login(page) -> bool:
    if not USERNAME or not PASSWORD:
        raise EnvironmentError(
            "STATEWIDE_USERNAME and STATEWIDE_PASSWORD must be set as environment variables"
        )
    ...
    # Wait for redirect to consumer area (up to 30 s in CI)
    try:
...
def _row_html(r: dict, idx: int) -> str:
    ...
    def ss_link(path):
        if not path:
            return "—"
        p = Path(path)
        return f'<a href="{esc(str(p))}" target="_blank">screenshot</a>'
...
<head>
<meta charset="UTF-8">
<title>Statewide OLB CTA Links Report — PROD</title>
<style>
...
<body>
<header>
  <h1>Statewide OLB — Card Ad CTA Links Report &mdash; PROD</h1>
  <p>Generated: {now} &nbsp;|&nbsp; Links navigated directly (no click) to avoid analytics</p>
</header>
<div class="stats">
...
@pytest.mark.asyncio
async def test_statewide_olb_card_ad_go_to_cta_links_github_actions_PROD():
    """
    CI version: validates card ad CTA link URLs on the Statewide OLB PROD dashboard.
    Runs headless; credentials and OTP secret come from environment variables.
    """
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            assert login_ok, "Login failed — check STATEWIDE_USERNAME / STATEWIDE_PASSWORD / STATEWIDE_OTP_SECRET"
            ...
            # Two refreshes to ensure ads load
            for i in range(1, 3):
    ...
    print(f"\n{'='*80}")
    print("TEST SUMMARY — PRODUCTION (CI)")
    print(f"{'='*80}")

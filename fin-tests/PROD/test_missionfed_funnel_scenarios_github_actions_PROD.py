...
#        (was incorrectly using .mat-select-arrow-wrapper without the 'mdc' component)
#        This fixes timeout failures in GitHub Actions when waiting for the selector to be visible.
#        The correct Angular Material class includes 'mdc' (Material Design Components).
...
def compare_images(baseline_path: Path, actual_path: Path, diff_out_path: Path, rms_tolerance: float = 0.0) -> float:
    ...
    with Image.open(baseline_path).convert('RGBA') as b, Image.open(actual_path).convert('RGBA') as a:
        ...
        try:
            ...
            threshold = 10
            # Fixed: Use list() instead of deprecated getdata()
            # getdata() will be removed in Pillow 14 (2027-10-15)
            changed = sum(1 for v in list(boosted.getdata()) if v > threshold)
...
async def test_missionfed_hero_ad_generic(
    ...
    try:
        ...
        # Other scenarios also use it for consistency
        if cfg.name.startswith("hispanic"):

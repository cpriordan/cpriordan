...
class AuthenticatedContext:
    def __init__(self, context, username, password, browser):
...
# Updated browser fixture to return both context and credentials
@pytest_asyncio.fixture
async def browser(request):
    username = request.param.get("username")
    password = request.param.get("password")
    async with async_playwright() as playwright:
        browser_instance = await playwright.chromium.launch(headless=False, args=["--remote-debugging-port=9222"])
        context = await browser_instance.new_context(http_credentials={"username": username, "password": password})
        context.set_default_timeout(40000)
        yield AuthenticatedContext(context, username, password, browser_instance)

...
class CreateASingleScenarioPage:
    def __init__(self, page):
        ...
        self.create_a_scenario_link = page.get_by_text("Create a Single Scenario") # UPDATED THE LINK DUE TO NEW UI
        self.create_a_single_scenario_link = page.get_by_text("Create a Single Scenario").nth(0) #First instance
        # self.target_a_product_link = page.get_by_text("Target a Product").nth(0) # first instance
        # TARGET A PRODUCT IS NOW A CHECKBOX INSTEAD OF A LINK USING THE NEW UI
        ...
        # AUTO LOAN is changed to CAR LOAN FOR THE NEW UI
        # self.auto_loan_scenario_type_link = page.get_by_text("Auto Loan")
        self.auto_loan_scenario_type_link = page.get_by_text("Car Loan").nth(0) # SEcond instance
    ...
    def navigate_to_input_scenario_content(self):
        ...
        self.page.wait_for_load_state("networkidle")
        # LINK "Create a scenario" changed to "Create a Single Scenario" so change the link
        # Click on create a scenario link
        # self.create_a_scenario_link.click()
        # Click on create a single scenario link
        self.create_a_single_scenario_link.click()
        self.page.wait_for_load_state("networkidle")
        # Click the first "Target a Product" link
        # self.target_a_product_link.click()
        # CLICK NOW ON A CHECKBOX INSTEAD OF A LINK DUE TO UI CHANGES AND THEN CONTINUE BUTTON
...
class ScenarioSetupCompletePage:
    ...
    def get_link_of_preview_now_staging_button(self):
        ...
        # WAIT FOR THE BUTTON TO BE VISIBLE BEFORE GETTING HREF ATTRIBUTE
        button_locator = self.page.locator("a.btn:nth-child(2)")
        button_locator.wait_for(state="visible", timeout=10000)  # WAIT EXPLICITLY HERE
        href = button_locator.get_attribute("href")

...
class PublishUnpublishPage:
    ...
    def create_test_scenario(self, screenshots_directory):
        ...
        # Step 1: Click "Product" button card (Dark Matter UI: styled card, not a raw checkbox)
        target_product_checkbox = self.page.get_by_text("Product", exact=True)
        target_product_checkbox.wait_for(state="visible", timeout=30000)
        target_product_checkbox.click()
        self.page.wait_for_load_state("networkidle")
...
def test_publish_and_unpublish_campaign(admin_browser_context_sync):
    ...
    screenshots_directory = os.path.join(os.path.dirname(__file__), 'screenshots_adminsite_using_pytest/publish_unpublish_campaign/')
